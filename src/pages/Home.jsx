...
import { downloadCsv, setExpenses, tryPremium } from "../redux/slice/expenseSlicer";
import { toogleTheme } from "../redux/slice/themeSlicer";
...
const Home = () => {
  ...
  const isPremium = useSelector(state => state.expenses.premium)
  ...
  const loadExpenses = async () => {
    try {
      ...
      for (let key in res.data) {
        total += Number(res.data[key].amount)
        expensesArr.push({ key: key, ...res.data[key] });
      }

      dispatch(setExpenses({ expensesArr, total }))
      setIsLoading(false);
    } catch (err) {
      setIsLoading(false);
      alert(err.message);
  ...
  return (
    <>
      ...
      <div className={`${themeMode ? "darkTheme" : "lightTheme"} `} style={{ height: '100vh' }}>
        <div className={`p-3 border-bottom border-black d-flex justify-content-between align-items-center `}>
          <h2>  Welcome to expense Tracker</h2>

          <div>
            {isPremium && (themeMode ? <i onClick={() => dispatch(toogleTheme())} className="bi bi-moon-fill cursor-pointer mx-3 fs-4"></i> : <i onClick={() => dispatch(toogleTheme())} className="bi bi-brightness-high-fill cursor-pointer mx-3 fs-4"></i>)}
        ...
        <div className="container mt-5">
          ...
          {isPremium &&
            <button className="btn btn-primary" onClick={()=>dispatch(downloadCsv())}>Download Csv</button>}

            {/* card  */}
            {isLoading && <p>Loading... Expenses</p>}
            {expenses.length === 0 && <p>No expenses to show ...</p>}
            {(totalExpense>=10000  && !isPremium) ?
             <button className="btn btn-primary" onClick={()=>dispatch(tryPremium())}>Try Premium</button>
             :
